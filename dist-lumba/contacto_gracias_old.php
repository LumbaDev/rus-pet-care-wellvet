...
$ramo = $input['ramo'] ?? 8; // Default value if not provided
...
if(curl_errno($curl)){
    ...
    curl_close($curl);
    echo json_encode(['status' => 'error', 'message' => 'Error sending to CRM: ' . $error_msg]);
} else {
    curl_close($curl);
    echo json_encode(['status' => 'success', 'data' => $crmResponse]);
}
?>
